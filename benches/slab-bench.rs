use allocators::slab::{
    std_impl::{MultiThreadedBench, StdEnv},
    MemCache,
};
...
#[inline]
fn dummy_mem_cache(cpu_count: usize) -> MemCache<StdEnv> {
    let utils = StdEnv::new(10240);
                    ...
                    for _ in 0..iters {
                        ...
                        let elapsed = bench
                            .thread(move |start, slab| {
                                start.wait();
                                let v: Vec<_> =
                                    (0..objects).map(|_| unsafe { slab.allocate() }).collect();
                                for obj in v {
